...
const parseString = (string) => {
    ...
    const year = match[1];
    const title = string.replace(regex, "").replace(/[^A-Za-z0-9]/g, ' ').replace(/\s+/g, " ").trim();
    return {year, title};
};
...
const scanShows = () => new Promise( async (res, rej) => {
    ...
    try{
        ...
        for(const folder of folders){
            ...
            const episodesFiles = fs.readdirSync(path).filter(file => validExt(file));
            const episodesArray = []
...
const getMoviePath = (mediaId) => new Promise((res, rej) => db.get('SELECT PATH FROM media_main WHERE MEDIA_ID = ?', [mediaId], (err, row) => err ? rej(err) : res(row ? row.PATH : undefined)));
const getEpisodePath = (episodeId) => new Promise((res, rej) => db.get(`SELECT PATH FROM episodes_main WHERE EPISODE_ID = ?`, [episodeId], (err, row) => err ? rej(err) : res(row ? row.PATH : undefined))); 
