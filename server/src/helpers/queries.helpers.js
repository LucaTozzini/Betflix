...
const availableGenres = () =>
  new Promise((res, rej) =>
    db.all("SELECT GENRE_NAME FROM genres", (err, rows) =>
    ...
    db.get(
      `SELECT 
				...
				genres AS (
					SELECT GENRE_NAME 
					FROM genres 
					JOIN media_genres ON genres.GENRE_ID = media_genres.GENRE_ID
			...
			WHERE main.MEDIA_ID = ?`,
      [mediaId, mediaId, userId, mediaId],
      (err, row) => (err ? rej(err) : res(row))
    )
  );

const queryGenre = (genreName, type, orderBy, limit) =>
  new Promise((res, rej) =>
    db.all(
