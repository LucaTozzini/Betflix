...
const Item = () => {
        ...
        if (len > 0) {
          async function get(index) {
            return new Promise(async (res) => {
              ...
              res({
                name: person.NAME,
                filmography: json.filter((i) => i.MEDIA_ID != mediaId),
              });
            });
          }
  ...
  const FetchGenre = async () => {
    try {
      ...
      const response = await fetch(
        `${serverAddress}/browse/genres?genreName=${name}`
      );
      const json = await response.json();
      setGenreMedia({
        name,
        items: json.filter((i) => i.MEDIA_ID != data.MEDIA_ID),
      });
    } catch (err) {
      console.error(err.message);
    }
  ...
  const Selector = () => {
    const options = data.AVAILABLE_SEASONS.filter(
      (i) => i.SEASON_NUM != currentSeason
    ).map((i) => ({
      value: i.SEASON_NUM,
    ...
    if (
      ...
      data.AVAILABLE_SEASONS.length > 1
    )
      return (
        <Select
          options={options}
  ...
  const PlayButtonText = () => {
    if (data.TYPE == 2) {
      if (currentEpisode) {
        return (
          <>
            S{currentEpisode.SEASON_NUM}:E{currentEpisode.EPISODE_NUM}
          </>
        );
      }
    }
    if (data.TYPE == 1) {
      if (data && resumeTime && resumeTime > 0) {
        ...
        const string = `${hr > 0 ? `${hr}h` : ""}${
          hr > 0 && (mn > 0 || sc > 0) ? ", " : ""
        }${mn > 0 ? `${mn}m` : ""}${
          mn > 0 && sc > 0 && mn == 0 && hr == 0 ? ", " : ""
        }${sc > 0 && mn == 0 && hr == 0 ? `${sc}s` : ""} Left`;
    ...
    return (
      <div className={styles.container}>
        ...
        <div
          ...
          style={{ backgroundImage: `url(${data.BACKDROP_L})` }}
        >
          <div className={styles.heroOverlay}>
            <img
              className={styles.logo}
              src={data.LOGO_L}
              style={{ opacity: data.LOGO_L ? 1 : 0 }}
            />
            <InfoBar />
        ...
        <div className={styles.infoSection}>
          ...
          {moreWithStar ? (
            <MediaSection
              key={"More_With_1"}
              title={`More With: ${moreWithStar.name}`}
              items={moreWithStar.filmography}
            />
          ) : (
            <></>
          )}
          {moreWithOther ? (
            <MediaSection
              key={"More_With_2"}
              title={`More With: ${moreWithOther.name}`}
              items={moreWithOther.filmography}
            />
          ) : (
            <></>
          )}
          {genreMedia ? (
            <MediaSection
              key={"More_In_1"}
              title={`More In: ${genreMedia.name}`}
              items={genreMedia.items}
            />
          ) : (
            <></>
          )}
